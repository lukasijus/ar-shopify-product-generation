...
const port = Number(process.env.NAIL_VISIBILITY_TRAINING_PORT ?? 5173);
const baseUrl = `http://127.0.0.1:${port}`;
const outputPath = join(root, "public/models/nail-visibility.training.json");
...
try {
  for (const fixtureId of fixtureIds) {
    ...
    await page.waitForFunction(
      (expectedFixtureId) =>
        window.__alwaysLikeVisibilityTrainingSample?.fixtureId ===
        expectedFixtureId,
      fixtureId,
      { timeout: 90_000 },
    );
  ...
  `${JSON.stringify(
    {
      ...
      source: "fixture-mediapipe-browser-feature-export",
